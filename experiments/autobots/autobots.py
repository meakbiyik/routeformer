...
class AutoBotEgo(nn.Module):
    ...
    def __init__(self, d_k=128, _M=5, c=5, T=30, L_enc=1, dropout=0.0, k_attr=2, map_attr=3,
        ...
        # ============================== Positional encoder ==============================
        self.pos_encoder = PositionalEncoding(d_k, dropout=0.0, max_len=100)
    ...
    def process_observations(self, ego):
        ...
        # Agents stuff
        temp_masks = torch.ones_like(env_masks_orig.unsqueeze(-1))
        opps_masks = (1.0 - temp_masks).type(torch.BoolTensor).to(ego.device)  # only for agents.

        return ego_tensor, opps_masks, env_masks
    ...
    def forward(self, ego_in, agents_in, roads):
        ...
        ego_tensor, opps_masks, env_masks = self.process_observations(ego_in)
        agents_tensor = ego_tensor.unsqueeze(2)
...
# autobot that outputs a sampled trajectory from its forward method (B, T, 2)
class AutoBotAdapted(AutoBotEgo):
    def __init__(self, configs: RouteformerConfig):
        self.configs = configs
        super().__init__(
            d_k=configs.encoder_hidden_size,
            _M=0, c=5, T=configs.gps_backbone_config.pred_len, L_enc=configs.gps_backbone_config.e_layers,
            dropout=0.0, k_attr=2, map_attr=3,
            num_heads=configs.encoder_heads,
            L_dec=configs.gps_backbone_config.d_layers,
            tx_hidden_size=configs.encoder_d_ff,
    ...
    def forward(self, batch, eval=False):
        ...
        motions = torch.cat([motions, torch.ones_like(motions[:, :, :1])], dim=2) # existence mask

        out_dists, mode_probs = super().forward(motions, None, None)

        # Extract mean positions from out_dists
        x_mean = out_dists[:, :, :, 0]  # [c, T, B]
        y_mean = out_dists[:, :, :, 1]  # [c, T, B]

        # Mode probabilities
        mode_probs = mode_probs.transpose(0, 1).unsqueeze(1)  # [c, 1, B]

        # Compute expected positions
        expected_x = (x_mean * mode_probs).sum(dim=0).permute(1, 0)  # [B, T]
        expected_y = (y_mean * mode_probs).sum(dim=0).permute(1, 0)  # [B, T]

        # Stack expected positions to form trajectories
        trajs = torch.stack([expected_x, expected_y], dim=2)  # [B, T, 2]

        # Add cumulative sum to last input GPS position
        last_input_gps = gps[:, -1:, :]
        future_gps_positions = last_input_gps + torch.cumsum(trajs, dim=1)
